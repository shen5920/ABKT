    ...
    def train(self):
        ...
        for e in range(self.epoch):
            ...
            if AUC > self.bestAUC:
                ...
                # 根据是否k折决定保存路径
                if self.use_kfold:
                    save_path_k = f'{save_dir}/GMF-boosting-{self.combine}-{self.embedding_k}-fold{self.fold_idx}-earlystop'
                else:
                    save_path_k = f'{save_dir}/GMF-boosting-{self.combine}-{self.embedding_k}-earlystop'
